...
describe('UsersController', () => {
  ...
  it('create debe retornar SuccessResponseDto', async () => {
    const response = await controller.create({ username: 'john', email: 'john@example.com', password: '123456' } as any);
  ...
  it('update debe lanzar NotFoundException si no existe', async () => {
    ...
    await expect(controller.update('no-id', { email: 'x@y.com' } as any)).rejects.toBeInstanceOf(NotFoundException);
  });

  describe('uploadProfile', () => {
    it('debe lanzar BadRequestException si no hay archivo', async () => {
      await expect(controller.uploadProfile('uuid-1', undefined as any)).rejects.toBeInstanceOf(BadRequestException);
    });

    it('debe lanzar NotFoundException si no existe el usuario', async () => {
      service.updateProfile.mockResolvedValueOnce(null as any);
      await expect(controller.uploadProfile('no-id', { filename: 'img.png' } as any)).rejects.toBeInstanceOf(NotFoundException);
    });

    it('debe retornar SuccessResponseDto si actualiza profile', async () => {
      const response = await controller.uploadProfile('uuid-1', { filename: 'img.png' } as any);
      expect(response?.success).toBe(true);
